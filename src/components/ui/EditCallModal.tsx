...
const STATUTS = [
  ...
  { value: "MANQUE",   label: "Manqué",   cls: "border-red-400 bg-rose-50 text-red-800"         },
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-slate-900">Modifier l'appel importé</h2>
            <p className="text-xs text-slate-400 mt-0.5">Seuls les appels importés peuvent être modifiés</p>
          </div>
        ...
        {/* Body */}
        {loadingData ? (
          <div className="flex-1 flex items-center justify-center p-8 text-slate-400 text-sm animate-pulse">
            Chargement...
          </div>
        ) : (
          <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">N° appelant *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Ligne</label>
                  ...
                  {STATUTS.map((s) => (
                    <button key={s.value} type="button" onClick={() => setStatut(s.value)}
                      className={`flex-1 py-2 rounded-lg border-2 text-sm font-medium transition-all ${
                        statut === s.value ? s.cls : "border-slate-200 bg-white text-slate-700 hover:border-slate-300"
                      }`}>
                      {s.label}
                    </button>
                  ))}
            ...
            {/* Footer */}
            <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between flex-shrink-0 gap-2">
              <button
                type="button"
