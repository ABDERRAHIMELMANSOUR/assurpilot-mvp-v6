    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-700">Tous les utilisateurs</h2>
          <span className="text-xs text-slate-400">{users.length} au total</span>
        </div>
        {loading ? (
                ...
                <tr key={u.id} className={`hover:bg-slate-50 ${!u.isActive ? "opacity-50" : ""}`}>
                  <td className="table-td">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-brand-50 flex items-center justify-center text-xs font-semibold text-brand-700">
                        {u.prenom[0]}{u.nom[0]}
                      </div>
                      <Link
                  ...
                  <td className="table-td font-mono text-sm text-slate-700">
                    {u.phoneNumber || <span className="text-slate-300 italic text-xs">—</span>}
                  </td>
                  <td className="table-td">
                  ...
                  <td className="table-td text-xs text-slate-500">{u.team?.nom ?? "—"}</td>
                  <td className="table-td">
