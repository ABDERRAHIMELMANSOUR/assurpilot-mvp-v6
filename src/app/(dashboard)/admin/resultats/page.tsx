...
const COLORS = [
  ...
  { value: "blue",   label: "Bleu",   cls: "bg-brand-50 text-brand-800 border-blue-300" },
  ...
  { value: "gray",   label: "Gris",   cls: "bg-slate-100 text-slate-800 border-slate-300" },
];
...
export default function ResultatsPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-slate-900">Résultats d'appel</h1>
      ...
      <div className="card overflow-hidden mb-6">
        <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-700">Options configurées</h2>
          <span className="text-xs text-slate-400">{options.filter(o=>o.isActive).length} active{options.filter(o=>o.isActive).length > 1 ? "s" : ""} / {options.length} total</span>
        </div>
        ...
        {loading ? (
          ...
          <p className="p-6 text-sm text-slate-400 text-center">Aucune option configurée.</p>
        ) : (
              ...
              <div key={opt.id} className={`px-5 py-3 flex items-center gap-3 ${!opt.isActive ? "opacity-50" : ""}`}>
                ...
                <div className="flex flex-col gap-0.5">
                  <button onClick={() => handleMove(opt, "up")} disabled={idx === 0}
                    className="w-5 h-4 flex items-center justify-center text-slate-400 hover:text-slate-700 disabled:opacity-30 disabled:cursor-not-allowed text-xs">▲</button>
                ...
                {editId === opt.id ? (
                  ...
                  <div className="flex flex-1 items-center gap-2">
                    <input value={editLabel} onChange={(e) => setEditLabel(e.target.value)}
                      className="flex-1 px-2.5 py-1.5 border border-blue-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" />
                    <select value={editColor} onChange={(e) => setEditColor(e.target.value)}
                      className="px-2 py-1.5 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-400">
      ...
      <div className="card p-5">
        <h2 className="text-sm font-semibold text-slate-700 mb-4 pb-2 border-b border-slate-100">Ajouter une option</h2>
        <form onSubmit={handleCreate} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Valeur (auto)</label>
              <input value={newValue} onChange={(e) => setNewValue(e.target.value)} placeholder="DEVIS_REALISE"
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-400 bg-slate-50" />
            </div>
          </div>
          <div>
