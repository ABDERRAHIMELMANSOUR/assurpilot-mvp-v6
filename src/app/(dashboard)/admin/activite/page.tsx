...
function dot(last: string | null) {
  if (!last) return { cls: "bg-slate-300", label: "Jamais connecté" };
...
export default function AdminActivitePage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-slate-900">Activité des utilisateurs</h1>
        <p className="text-sm text-slate-500 mt-0.5">Dernières connexions et historique d'activité</p>
      </div>
        ...
        <div className="card overflow-hidden">
          <div className="divide-y divide-slate-100">
            {users.map((u) => {
              ...
              return (
                <div key={u.id}>
                  <div className="px-5 py-3.5 flex items-center gap-3 hover:bg-slate-50 cursor-pointer transition-colors"
                    onClick={() => setExpanded(open ? null : u.id)}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${u.isActive ? "bg-brand-50 text-brand-700" : "bg-slate-100 text-slate-500"}`}>
                      {u.prenom[0]}{u.nom[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 font-mono truncate">{u.phoneNumber || u.email}</p>
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <span className={`w-2.5 h-2.5 rounded-full ${d.cls}`} title={d.label} />
                      <span className="text-xs text-slate-500 hidden sm:block">{d.label}</span>
                    </div>
                    <div className="text-right flex-shrink-0 min-w-[100px]">
                      {u.lastLoginAt ? (
                        ...
                        <p className="text-xs text-slate-400 italic">Jamais connecté</p>
                      )}
                    </div>
                    ...
                    <div className="px-5 pb-4 bg-slate-50 border-t border-slate-100">
                      ...
                      {u.loginLogs.length === 0 ? (
                        ...
                        <div className="space-y-1.5">
                          {u.loginLogs.map((log) => (
                            <div key={log.id} className="flex items-center gap-3 text-xs text-slate-600">
                              ...
                              {log.ip && <span className="text-slate-400 font-mono">{log.ip}</span>}
                            </div>
                          ))}
                        </div>
                      )}
