  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-5 flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Mes statistiques</h1>
          <p className="text-sm text-slate-500 mt-0.5">Votre performance sur la période sélectionnée</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            <StatCard label="Total appels" tone="brand"        value={stats.total} />
            <StatCard label="Répondus" tone="emerald"             value={stats.repondus}   subColor="text-emerald-600" />
            <StatCard label="Manqués" tone="rose"              value={stats.manques}    subColor="text-rose-500" />
            <StatCard label="Devis réalisés" tone="emerald"       value={stats.devis}      subColor="text-emerald-600" />
            <StatCard label="Taux de conversion" tone="indigo"
              value={`${stats.tauxConversion}%`}
              sub="sur appels répondus"
              subColor={stats.tauxConversion >= 30 ? "text-emerald-600" : "text-amber-600"} />
          ...
          <div className="card p-5">
            <h3 className="text-sm font-semibold text-slate-700 mb-4">Répartition des résultats</h3>
            <div className="space-y-3">
              {[
                { label: "Devis réalisés",  count: stats.devis,                        color: "bg-emerald-500", total: stats.repondus },
