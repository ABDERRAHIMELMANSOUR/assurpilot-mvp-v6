  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-5 flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Vue d'ensemble — Équipe</h1>
          <p className="text-sm text-slate-500 mt-0.5">Performances globales de votre équipe</p>
        </div>
              ...
              {(stats.leaderboard ?? []).map((agent: any, i: number) => {
                ...
                return (
                  <div key={agent.id} className="px-5 py-3.5 flex items-center gap-4">
                    <span className="text-sm font-semibold text-slate-300 w-5 text-center">{i + 1}</span>
